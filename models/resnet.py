...
class ResnetExperiment(WrapperModule):
    def __init__(self):
        ...
        self.opt = SGD(self.resnet.parameters(), lr=0.1,
                                momentum=0.9,
                                weight_decay=1e-4)
        self.loss_fun = nn.CrossEntropyLoss()
        self.train_acc = MulticlassAccuracy(num_classes=10)
        self.distribution_ignore = ['loss_fun', 'train_acc']
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.opt,milestones=[100,150])
    ...
    def on_test_end(self,test_results):
        self.log_dict({'test_acc': self.train_acc.compute()},
                      on_step=True, on_epoch=False)  # add auto epoch adding feature
        self.train_acc.reset()
